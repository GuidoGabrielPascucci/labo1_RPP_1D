	...
	do
		{
			option = getIntInMinMaxRange("\n_________________________________________________________________________________________________________________________________\n\n\n"
										 ...
										 "9. Alta de perro\n"
										 "10. Cantidad de dueños que se llaman Alan y que, a su vez, tienen al menos una estadía reservada\n"
			...
			switch (option)
			{
				...
				case 5:
					sPerro_listarPerros(listaPerros, lengthPerros);
					break;
				...
				case 9:
					sPerro_agregarPerrito(listaPerros, lengthPerros, &contadorIdPerros);
					break;

				case 10:
					calcularYMostrarCantidadDeDueniosAlan(listaDuenios, listaEstadias, lengthDuenios, lengthEstadias, contadorAltas);
					break;

				case 11:
					calcularYMostrarEstadiasDeLuciaEnSegundaQuincenaDeNoviembre2021(listaDuenios, listaEstadias, listaPerros, lengthDuenios, lengthEstadias, lengthPerros, contadorAltas);
...
{
	int acumuladorAlan = 0;

	if (contadorAltas > 0)
	{

		if ( (listaDuenios != NULL && listaEstadias != NULL) && (lengthDuenios > -1 && lengthEstadias > -1) )
		{

			for (int i = 0; i < lengthDuenios; ++i)
			{
				if (listaDuenios[i].espacioVacio == FALSE)
				{
					for (int j = 0; j < lengthEstadias; ++j)
					{
						if (listaEstadias[j].espacioVacio == FALSE && listaDuenios[i].id == 30000 && listaEstadias[j].idDuenio == 30000)
						{
							acumuladorAlan++;
						}
					}
			...
			if (acumuladorAlan > 0)
			{
				printf("\n\nCANTIDAD DE DUENIOS ALAN: %d\n\n\n", acumuladorAlan);
			}
			else
			{
				printf("\n\nNo hay estadias de Alan\n\n\n");
			}
		...
		if ( (listaDuenios != NULL && listaEstadias != NULL) && (lengthDuenios > -1 && lengthEstadias > -1) )
		{
			...
			for (int i = 0; i < lengthDuenios; ++i)
			{
				if (listaDuenios[i].espacioVacio == FALSE)
				{
					for (int j = 0; j < lengthEstadias; ++j)
					{
						if (listaEstadias[j].espacioVacio == FALSE && listaDuenios[i].id == 30001 && listaEstadias[j].idDuenio == 30001 && listaEstadias[j].fecha.anio == 2021 && listaEstadias[j].fecha.mes == 11 && listaEstadias[j].fecha.dia > 15)
						{
							acumuladorLucia++;
						}
					}
			...
			if (acumuladorLucia > 0)
			{
				printf("\n\nCANTIDAD DE DUENIAS LUCIA EN SEGUNDA QUINCENA DE NOVIEMBRE ANIO 2021: %d\n\n\n", acumuladorLucia);

				//mostrarEstadiasDeLuciaSegundaQuincenaNoviembre2021(listaDuenios, listaEstadias, listaPerros, lengthDuenios, lengthEstadias, lengthPerros);

				mostrarEstadiasDeLuciaSegundaQuincenaNoviembre2021(listaDuenios, listaEstadias, lengthDuenios, lengthEstadias);
			}
			else
			{
				printf("\n\nNo hay estadias de Lucia cargadas en la segunda quincena de noviembre anio 2021\n\n\n");
			}
...
void mostrarEstadiasDeLuciaSegundaQuincenaNoviembre2021(sDuenio* listaDuenios, sEstadiaDiaria* listaEstadias, int lengthDuenios, int lengthEstadias)
{
	...
	for (int i = 0; i < lengthDuenios; ++i)
	{
		if (listaDuenios[i].espacioVacio == FALSE && listaDuenios[i].id == 30001)
		{

			printf("\n\nEstadias de %s:\n"
					"---------------------------------------------------------------------------------------------------------------------------------\n", listaDuenios[i].nombre);
			...
			for (int j = 0; j < lengthEstadias; ++j)
			{
				if (listaEstadias[i].espacioVacio == FALSE && listaDuenios[i].id == listaEstadias[j].idDuenio && listaEstadias[j].fecha.anio == 2021 && listaEstadias[j].fecha.mes == 11 && listaEstadias[j].fecha.dia > 15)
				{
					printf("%-20s %-20d %d/%d/%d\n", listaDuenios[i].nombre,
													 listaDuenios[i].telefono,
													 listaEstadias[j].fecha.dia,
													 listaEstadias[j].fecha.mes,
													 listaEstadias[j].fecha.anio);
				}
